...
/** Wires every `stream_chunk`/`stream_end`/`agent_*` WebSocket event (published by
 *  `apps/backend/app/api/ws/publisher.py`, Phase 7's gateway) into the chat/agent store slices'
 *  handlers. Mounted once at the top of the app (not inside `ChatPanel`/`AgentPanel`) so a task
 *  running while the user is looking at a different sidebar view still updates state — switching
 *  back to Chat or Agent Tasks shows the current state immediately instead of only whatever
 *  arrived while that panel happened to be mounted. */
