...
- [x] **Resolved, found already built 2026-08-13:** LSP completion/code-action providers —
  ...
  claiming it as new.
- [x] **Resolved 2026-08-13 (part 7):** Phase 8's `get_diagnostics` agent tool is now real — not
  by wiring into the desktop's `lsp-manager.ts` (an Electron/TS client in a different app this
  Python backend can't reach), but via a new, separate backend-side LSP client
  (`app/infrastructure/lsp/client.py` + `manager.py`, Python-only via `pylsp`, real-verified
  against a real spawned `pylsp` process). Registered for `coder`/`debugger`/`reviewer` agents. See
  `AGENT_FRAMEWORK.md`'s "Available Tools" section and `PROGRESS.md`'s Phase 8 entry for the full
  writeup. TypeScript/JSON diagnostics remain open — see the new item below.
...
- [x] **Resolved 2026-08-12:** `MonacoEditor` — see the dedicated "monaco-editor Vitest blocker" entry below for the real fix and what it unblocked. **Design-system primitives corrected 2026-08-14:** all 9 of 9 (`Input`, `Tooltip`, `Dialog`, `ScrollArea`, `Badge`, `ContextMenu`, plus the 3 already credited) now have real, already-committed test files (`apps/desktop/src/components/ui/{Input,Tooltip,Dialog,ScrollArea,Badge,ContextMenu}.test.tsx`, verified by direct read — e.g. `Dialog.test.tsx` has 6 real tests covering open/closed rendering, footer, close-button click, and Escape dismissal) — this "6 of 9 missing" line was itself stale, another instance of code being done before tracking caught up. Still remaining, not reverified this pass: `file-handlers.ts`/`shell-handlers.ts` IPC modules.
...
- [x] **Resolved, found already built 2026-08-13 (part 7):** stale-connection idle timeout is test-verified for real — `gateway.py` reads `settings.ws_idle_timeout_seconds` (already settings-driven, not the hardcoded constant this item's prior wording assumed), and `tests/integration/conftest.py`'s `live_server_short_idle_timeout` fixture (`WS_IDLE_TIMEOUT_SECONDS=1` via env var) backs two real passing tests: close-after-idle and ping-resets-the-timer, both against a real running server in well under a second. Not new work — crediting real prior work this file hadn't caught up to, the same drift pattern hit repeatedly elsewhere in this project.
...
- [x] **Resolved 2026-08-13 (part 7):** `lsp_tools.py` (`get_diagnostics`) — real backend-side LSP client, Python only. See the dedicated entry above.
- [ ] `get_diagnostics` covers Python only (`pylsp`) — TypeScript/JSON diagnostics would need either vendoring `typescript-language-server`/`vscode-json-language-server` as a real backend dependency (they currently exist only as `apps/desktop`'s own npm devDependencies, not reachable from the Python backend without duplicating them) or a new API surface the desktop's already-running LSP client answers through instead. Not started — a real, separate follow-up, not silently folded into the Python-only build.
...
- [x] `MonacoEditor` — resolved 2026-08-12, see the dedicated "monaco-editor Vitest blocker" entry below. **Design-system primitives:** all 9 of 9 now have real test coverage (corrected 2026-08-14, see the Phase 3 entry above). Still open: `file-handlers.ts`/`shell-handlers.ts`.
...
- [x] **Resolved 2026-08-13 (part 7):** a full broken-link audit across the entire repository — all 135 markdown files (21 root docs + everything under `docs/`), not just files a given session created/modified. Zero broken links found.
...
- [x] **Corrected 2026-08-14:** this line was stale — `git log` now shows 6 commits (not 2), including a 2026-08-13 `7f6e5cb "Rasik_studio"` commit that already covers prior sessions' work (PROGRESS.md's own part-6 entry already credited this; this line at the bottom of the file was simply never revised to match). The general point still applies going forward, though: the working tree currently has real uncommitted changes again (the LSP client work — `apps/backend/app/infrastructure/lsp/`, `agents/tools/lsp_tools.py`, and their tests) sitting alongside modified tracking docs. Per `CLAUDE.md`'s Development Rules, the user commits, not Claude — this item just tracks that a commit is due, not that one is overdue by sessions' worth of work.
